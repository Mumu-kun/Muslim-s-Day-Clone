...
void iFilledSector(double x, double y, double r, double sAngle, double diffAngle, int slices = 100)
{
    ...
    // xp = x;
    // yp = y + r;
    for (t = sAngle; t <= sAngle + diffAngle; t += dt)
    {
        x1 = x - r * cos(t);
        y1 = y + r * sin(t);
...
void mousePassiveHandlerFF(int mx, int my)
{
    iMouseX = mx;
    iMouseY = iScreenHeight - my;
    iMousePos(iMouseX, iMouseY);
...
void iInitialize(int width = 500, int height = 500, char *title = "iGraphics")
{
    ...
    glutIdleFunc(animFF);
    glutPassiveMotionFunc(mousePassiveHandlerFF);
